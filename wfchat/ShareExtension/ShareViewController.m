...
- (void)viewDidLoad {
    ...
    if (![[ShareAppService sharedAppService] isLogin]) {
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"没有登录" message:@"请先登录" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)showImageLimit {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"不支持发送多张图片" message:@"每次只能发送一张。如果您需要一次发送多张，请打开聊天室选择图片发送。" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)showSuccess {
    ...
    __weak typeof(self)ws = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"已发送" message:@"您可以在程式中查看" preferredStyle:UIAlertControllerStyleAlert];
