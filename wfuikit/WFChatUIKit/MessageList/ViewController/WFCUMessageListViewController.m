...
- (void)setupNavigationItem {
    if (self.multiSelecting) {
        ...
        if (self.conversation.type == Chatroom_Type) {
            ...
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"详情" style:UIBarButtonItemStyleDone target:self action:@selector(onRightBarBtn:)];
            [self.navigationItem.rightBarButtonItem setTitleTextAttributes:@{NSForegroundColorAttributeName : [UIColor colorWithHexString:@"#4970BA"]} forState:UIControlStateNormal];
        }
    }
}
...
- (void)updateTitle {
    if(self.conversation.type == Single_Type || self.conversation.type == SecretChat_Type) {
        ...
        if(self.targetGroup.displayName.length == 0) {
            ...
            if ([WFCUUtilities isBroadcastGroup:self.targetGroup.extra]) {
                self.title = [NSString stringWithFormat:@"%@", self.targetGroup.displayName];
            } else {
                UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, UIScreen.mainScreen.bounds.size.width * 0.6, 20)];
                label.text = self.targetGroup.displayName;
                [WFCUUtilities addAndTruncateStringWithLabel:label resevedString:[NSString stringWithFormat:@"(%ld)", self.targetGroup.memberCount]];
...
- (void)initializedSubViews {
    ...
    [self registerCell:[WFCUInformationCell class] forContent:[WFCCSensitiveWordContent class]];
...
- (void)onSendingMessage:(NSNotification *)notification {
    ...
    [self processMessageError:[notification.userInfo objectForKey:@"errorCode"] message:message];
}

- (void)processMessageError:(NSNumber *)errorCode message:(WFCCMessage *)message{
    ...
    switch (errorCode.intValue) {
        case ERROR_CODE_SENSITIVE_MATCHED:
            [self loadRemoteErrorMessage];
            break;
        case ERROR_CODE_SLIENCE: {
            [self.chatInputBar resetInputBarStatue];
            [self.view makeToast:@"发言太过频繁，禁止发言30秒"];
            break;
        }
        default:
            break;
...
- (void)loadRemoteErrorMessage {
    [[WFCCIMService sharedWFCIMService] getRemoteMessages:self.conversation before:LONG_LONG_MAX count:10 contentTypes:nil success:^(NSArray<WFCCMessage *> *messages) {
        NSMutableArray *reversedMsgs = [[NSMutableArray alloc] init];
        for (WFCCMessage *msg in messages) {
            [reversedMsgs insertObject:msg atIndex:0];
            if (msg.messageUid > 0 && msg.messageUid < self.lastUid) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self appendMessages:reversedMsgs newMessage:YES highlightId:0 forceButtom:NO];
        });
    } error:^(int error_code) {
    }];
}
