...
- (void)updateTitle {
    ...
    if (status != kConnectionStatusConnecting && status != kConnectionStatusReceiving) {
        UILabel *navLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2 - 40, 0, [UIScreen mainScreen].bounds.size.width, 44)];
        ...
        navLabel.textColor = [WFCUConfigManager globalManager].naviTextColor;
        navLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:24];
...
- (void)refreshList {
    ...
    [self getBroadCastGroup];
    [self updateBadgeNumber];
    [self.tableView reloadData];
}

- (void)getBroadCastGroup {
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    hud.label.text = @"讀取中...";
    [hud showAnimated:YES];
    
    [[WFCUConfigManager globalManager].appServiceProvider getGroupList:^(GroupListModel * _Nonnull model) {
        for (GroupListData *data in model.data) {
            if ([[WFCCIMService sharedWFCIMService] getConversationInfo:data.WFCCConversation].isTop == 0) {
                [[WFCCIMService sharedWFCIMService] setConversation:data.WFCCConversation top:1 success:nil error:^(int error_code) {
                }];
            }
        }
        ...
        NSArray<WFCCConversationInfo *> *conversation = [model createNoRepeatInfoWithConversationInfos:self.conversations];
        if (self.conversations == nil || conversation.count == 0) {
            return;
        }
            
        NSIndexSet *index = [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, conversation.count)];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.conversations insertObjects:conversation atIndexes:index];
            [self.tableView reloadData];
        });
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.searchController.active) {
        ...
        mvc.hidesBottomBarWhenPushed = YES;
        NSInteger count = 0;
        for (WFCCConversationInfo *i in self.conversations) {
            if (!i.isSilent && i != info) {
                count += i.unreadCount.unread;
            }
        }

        if (count > 0) {
            NavgationCountView *view = [[NavgationCountView alloc] init];
            [view.control addTarget:mvc action:@selector(popViewController) forControlEvents:UIControlEventTouchDown];
            view.title = [NSString stringWithFormat:@"%ld", count];
            
            UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithCustomView:view];
            mvc.navigationItem.leftBarButtonItem = item;
