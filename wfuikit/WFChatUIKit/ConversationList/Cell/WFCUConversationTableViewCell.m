...
- (void)setupUI {
    ...
    self.timeLabel.textColor = [UIColor colorWithHexString:@"0xadadad"];
    ...
    self.targetView = [[UILabel alloc] init];
    self.targetView.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:17];
    ...
    [self.targetView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.potraitView.mas_right).offset(10);
...
- (void)updateGroupInfo:(WFCCGroupInfo *)groupInfo {
    ...
    NSString *groupName = groupInfo.displayName.length > 0 ? groupInfo.displayName : WFCString(@"GroupChat");
    self.targetView.text = groupName;
    if (![WFCUUtilities isBroadcastGroup:groupInfo.extra]) {
        NSString *countString = [NSString stringWithFormat:@"(%ld)", groupInfo.memberCount];
        [self.contentView layoutIfNeeded];
        [WFCUUtilities addAndTruncateStringWithLabel:self.targetView resevedString:countString];
    }
}
...
- (void)setInfo:(WFCCConversationInfo *)info {
    ...
    [[WFCCIMService sharedWFCIMService] getRemoteMessages:self.info.conversation before:LONG_LONG_MAX count:1 contentTypes:nil success:^(NSArray<WFCCMessage *> *messages) {
        if (messages == nil || messages.count == 0) {
            return;
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            self.info.lastMessage = messages[0];
            [self updateUI];
        });
...
- (void)updateUI {
    if (self.info.unreadCount.unread == 0) {
        self.bubbleView.hidden = YES;
    } else {
        self.bubbleView.hidden = NO;
        if (self.info.isSilent) {
            self.bubbleView.isShowNotificationNumber = NO;
        } else {
            self.bubbleView.isShowNotificationNumber = YES;
        }
        [self.bubbleView setBubbleTipNumber:self.info.unreadCount.unread];
    }
    
    if (self.info.isSilent) {
        self.silentView.hidden = NO;
    } else {
        self.silentView.hidden = YES;
    }
  
    [self update:self.info.conversation];
    ...
    if (self.info.lastMessage && self.info.lastMessage.direction == MessageDirection_Send) {
        if (self.info.lastMessage.status == Message_Status_Sending) {
            self.statusView.image = [WFCUImage imageNamed:@"conversation_message_sending"];
            self.statusView.hidden = NO;
        } else if(self.info.lastMessage.status == Message_Status_Send_Failure) {
...
- (void)update:(WFCCConversation *)conversation {
    if (self.info.lastMessage != nil && self.info.lastMessage.serverTime != 0) {
        self.timeLabel.text = [WFCUUtilities formatTimeDetailLabel:self.info.lastMessage.serverTime];
    }
    ...
    if(conversation.type == SecretChat_Type) {
        ...
        self.targetView.frame = CGRectMake(16 + 48 + 12, 16, [UIScreen mainScreen].bounds.size.width - 76  - 40, 20);
    }
