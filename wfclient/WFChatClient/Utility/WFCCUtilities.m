...
+ (NSString *)replaceDomainWithString:(NSString *)string {
    if (![string containsString:@"{{domain}}"]) {
        return string;
    }
    
    NSString *domain = [[NSUserDefaults standardUserDefaults] objectForKey:@"mediaDomain"];
    if (domain == nil || string == nil || [string isEqualToString:@""]) {
        return string;
    }
    
    NSURLComponents *newComponent = [NSURLComponents componentsWithString:domain];
    
    if (newComponent == nil) {
        return string;
    }
    
    NSString *tempString = [string stringByReplacingOccurrencesOfString:@"{{domain}}" withString:@"https://127.0.0.1"];
    NSURLComponents *oldComponent = [NSURLComponents componentsWithString:tempString];
    
    if (oldComponent == nil) {
        return string;
    }
    
    if ([newComponent.path hasSuffix:@"/"]) {
        newComponent.path = [newComponent.path substringToIndex:newComponent.path.length - 1];
    }

    newComponent.path = [NSString stringWithFormat:@"%@%@",newComponent.path, oldComponent.path];
    
    return newComponent.URL.absoluteString;
}
